...
function insert_custom_block(listview) {
  ...
  if (!container) {
    setTimeout(() => insert_custom_block(listview), 500);
    return;
  }
  ...
  const blockHTML = `
    ...
    <div class="custom-list-container">
      ...
      <div class="custom-box">
        ...
        <div class="box-right">
          <img src="/assets/erptheme/images/unpaid.png" alt="icon" style="width: 32px; height: 32px;">
        </div>
      </div>
      <div class="custom-box">
        <div class="box-left">
        ...
        <div class="box-right">
          <img src="/assets/erptheme/images/draft.png" alt="icon" style="width: 32px; height: 32px;">
        </div>
      </div>
    </div>
  </div>
...
// ================= Update stats ====================
function update_purchase_invoice_stats() {
  frappe.call({
  ...
  frappe.db.count("Purchase Invoice", { filters: { outstanding_amount: [">", 0] } })
    .then(c => {
      const e = document.getElementById("unpaid_box");
      if (e) e.textContent = c;
    });

  frappe.db.count("Purchase Invoice", { filters: { docstatus: 0 } })
    .then(c => {
